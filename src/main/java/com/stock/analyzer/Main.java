...
public class Main {
        ...
        static final List<Integer> sectors = List.of(
                        ...
                        201010, 201020, 201030, 201040, 201050, 201060, 201070, 202010, 202020, 203010, 203020, 203030,
                        203040, 203050, // Industrials
        ...
        public static void main(String[] args) throws IOException {
                ...
                StockFetcherImpl stockFetcher = new StockFetcherImpl(baseURL, "{code}", headers);

                List<String> exchanges = List.of("TASE", "NYSE", "NasdaqGS", "NasdaqGM", "NasdaqCM", "LSE", "TSE",
                                "ENXTLS", "ENXTPA", "ENXTBR", "ARCA");
                StockFilterImpl stockFilter = new StockFilterImpl(exchanges, 50_000_000.0, 0);
                ...
                StockEnricherImpl stockEnricher = new StockEnricherImpl(
                ...
                StockGraphServiceImpl stockGraphService = new StockGraphServiceImpl(
                ...
                DailyAnalysisStrategy dailyStockAnalysisStrategy = new DailyAnalysisStrategy();
                ...
                try {
                        ...
                        for (int i = 0; i < sectors.size(); i += batchSize) {
                                ...
                                List<CompletableFuture<ExecutionResult>> futures = batch.stream()
                                                .map(sector -> CompletableFuture
                                                                .supplyAsync(() -> processor.getStockData(sector)))
                                                .toList();
                                ...
                                List<ExecutionResult> results = futures.stream()
                                                ...
                                                .toList();
                                System.out.println("Batch completed. Processed " + results.size() + " sectors.");
                        }
