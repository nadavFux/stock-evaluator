...
public class StatsCalculator {
    ...
    public static double calculateSlidingAvg(List<Double> list, int endIndex, int timeFrame, String key) {
        lock.lock();
        try {
            if (preComputedMoving.containsKey(key)) {
                return preComputedMoving.get(key);
            } else {
                double sum = 0.0;
    ...
    public static int[] findLowestAndHighest(List<Double> prices, int start, int end) {
        ...
        for (int i = start + 1; i < end; i++) {
            if (prices.get(i) < prices.get(lowest)) lowest = i;
            if (prices.get(i) > prices.get(highest)) highest = i;
        }
    ...
    public static List<Double> MovingAvg(StockGraphState stock, int period) {
        ...
        for (int i = 0; i < stock.closePrices().size(); i++) {
            if (i < period) {
                ...
                movingAverages.add(stock.closePrices().subList(i - period, i).stream().mapToDouble(price -> price).sum() / period);
            }
        }
    ...
    public static void WriteStat() {
        ...
        try (FileWriter writer = new FileWriter(filePath)) {
            // Write the data
            var sortedSimulation = new ArrayList<>(SIMULATIONS.values()).stream().sorted(Comparator.comparingDouble(Simulation::calculateSimulationScore)).toList();
    ...
    public static void WriteStatEvals() {
        ...
        try (FileWriter writer = new FileWriter(filePath)) {
            // Write the data
            var sortedSimulation = SIMULATIONS.values().stream().sorted(Comparator.comparingDouble(Simulation::getEval)).toList();
            for (Simulation simulation : sortedSimulation) {
